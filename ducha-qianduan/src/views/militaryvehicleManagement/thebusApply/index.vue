    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          {{
            parseTime(scope.row.authStime, "{h}:{i}:{s}")
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          ...
          {{
            parseTime(scope.row.authEtime, "{h}:{i}:{s}")
          }}
        </span>
        </template>
      </el-table-column>
      ...
      <template slot-scope="scope">
          ...
          {{
            parseTime(scope.row.battaliontime, "{h}:{i}:{s}")
          }}
        </span>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="车辆类别" prop="vehicleType">
          ...
          <!--            :disabled="dislable"-->
          <!--          />-->
          <el-select v-model="form.vehicleType" placeholder="请选择车辆类别" @change="handleVehicleTypeChange" :disabled="dislable">
        ...
        <el-form-item label="车牌号码" prop="carIds">
          <!--          <el-input-->
          <!--            v-model="form.model"-->
          <!--            placeholder="请输入用车型号"-->
          <!--            :disabled="dislable"-->
          <!--          />-->
          <el-select v-model="form.carIds" placeholder="请选择车牌号码" :disabled="dislable">
            <el-option
              ...
              :value="item.dictCode"
              class="input_xiala"
              @click.native="handleVehicleModel(item)"
            >
            </el-option>
          </el-select>
        </el-form-item>
...
<!--            :disabled="dislable"-->
<!--          />-->
<!--          <el-select v-model="form.model" placeholder="请选择用车型号" @change="handleModelChange" >-->
<!--            <el-option-->
<!--              v-for="(item, i) in vehicleList"-->
<!--              :key="i"-->
...
<!--            </el-option>-->
<!--          </el-select>-->
          <el-input v-model="form.modelLabel" placeholder="车型" disabled />
        </el-form-item>
...
<!--          />-->
        ...
        <el-form-item label="车牌号码" prop="carIds">
          ...
          <!--            :disabled="dislable"-->
          <!--          />-->
          <el-select v-model="form.carIds" placeholder="请选择车牌号码">
            <el-option
              v-for="(item, i) in brand"
              :key="i"
              :label="item.dictCode"
...
import {getZhuChiRen, getDept, getThebusType} from "@/api/militaryvehicleManagement/thebus";
import {
  listThebusConfig,
  getThebusUser,
...
import {getThePrivateBusType, listPrivateCar} from "@/api/militaryvehicleManagement/privateCar";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        model: [{ required: true, message: "请选择用车型号", trigger: "blur" }],
        carIds: [{ required: true, message: "请选择车牌号码", trigger: "blur" }],
  ...
  methods: {
    ...
    handleVehicleTypeChange(type) {
      if (type === "MILITARY") {
        ...
        // 私家车
        this.loadPrivateCarData();
      }
    },
    //选择车牌触发
    handleVehicleModel(data){
      this.$set(this.form, "modelLabel", data.dictLabel);
      this.$set(this.form, "model", data.dictValue);
    },
    handleModelChange(value) {
      // 根据选中的 dictValue 找到对应项
      const selectedVehicleName = this.brand.find(item => item.dictValue === value);
      if (selectedVehicleName) {
        this.form.carNames = selectedVehicleName.dictLabel; // 把 dictLabel 存进表单
      } else {
        this.form.carNames = "";
      }
    },
    ...
    // 车辆型号
    loadMilitaryData() {
      getThebusType().then(res => {
        this.applyCarTypeData(res.data || []);
      });
    },
    // getCatPai() {
    //   getThebusType().then((response) => {
    ...
    // 加载私家车数据
    loadPrivateCarData() {
      getThePrivateBusType().then(res => {
        this.applyCarTypeData(res.data || []);
      });
    },
    // 通用处理函数
    applyCarTypeData(list) {
...
::v-deep .btns-form-item .el-button {
  margin-right: 8px;
}

/* 批量删除按钮，防止文字挤出方框 */
.btn-batch-delete {
  min-width: 110px;      /* 或者 96/110 自己微调 */
  padding: 4px 16px;     /* 比默认多一点左右间距 */
  box-sizing: border-box;
}
</style>
