  ...
  <div class="app-container">
    ...
    >
      <el-form-item label prop="unitName">
        <el-input v-model="queryParams.unitName" placeholder="请输入单位" clearable size="small" />
      </el-form-item>
      <el-form-item label prop="job">
        <el-input v-model="queryParams.job" placeholder="请输入工作岗位" clearable size="small" />
      </el-form-item>
      <el-form-item label prop="classId">
        <el-select v-model="queryParams.classId" placeholder="请选择人员类别" clearable>
          <el-option
    ...
    <div class="hezi">
      ...
      <div style="display: flex; flex-wrap: wrap; padding: 11px 20px">
        <div style="width: 33.3%; display: flex; align-items: center; height: 100px">
          <div class="div-l" style="flex: 2; text-align: right; margin-right: 30px；height:30px;">
            <span>总体检人数</span>
          </div>
          <div class="div-r" style="flex: 1; text-align: right; margin-right: 180px;height:30px;">
            <span>
              <span style="font-size: 25px; margin-right: 18px">{{tongjilist.counts }}</span>人
            </span>
          </div>
        </div>
        <div style="width: 33.3%; display: flex; align-items: center; height: 100px">
          ...
          <div class="div-r" style="flex: 1; text-align: right; margin-right: 180px;height:30px;">
            <span>
              <span style="font-size: 25px; margin-right: 18px">{{ tongjilist.ytijian}}</span>人
            </span>
          </div>
        </div>
        <div style="width: 33.3%; display: flex; align-items: center; height: 100px">
          ...
          <div class="div-r" style="flex: 1; text-align: right; margin-right: 180px;height:30px;">
            <span>
              <span style="font-size: 25px; margin-right: 18px">{{tongjilist.wtj }}</span>人
            </span>
          </div>
        </div>
      </div>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['medicalhealth:people:Examinationedit']"
          >体检完成</el-button>
          <!-- <el-button
            size="btp"
            type="text"
    ...
    >
      <div class="ww">
        <el-form ref="form" :model="form" :rules="rules1" :inline="true" style="margin-left: 30px">
          ...
          <el-form-item>
            <el-input v-model="form.contents" type="textarea" placeholder style="width: 870px"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  created() {
    this.getList();
    this.getRenLei();
    this.getPeopleExaminationcount();
  },
  methods: {
    tijiann(row) {
      var myDate = new Date();
      row.job = myDate.getFullYear();
      Examinationedit(row).then(res => {
        if (res.code == 200) {
          this.$modal.msgSuccess("体检已成功");
          this.getList();
          this.getPeopleExaminationcount();
        }
      });
    ...
    getPeopleExaminationcount() {
      var myDate = new Date();
      this.queryParams.job = myDate.getFullYear();
      getPeopleExaminationcount({ job: myDate.getFullYear() }).then(res => {
        console.log(res);
        if (res.code == 200) {
          this.tongjilist = res.data;
          this.tongjilist.wtj = res.data.counts - res.data.ytijian;
        }
      });
    },
    // 删除
