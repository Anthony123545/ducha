        ...
        <div style="display: flex; height: 100%; width: 100%">
          <div style="flex: 0.4; height: 100%">
            <el-card>
              ...
              <!-- 视频流 -->
              <el-divider v-if="device && device.videosInfo && device.videosInfo.length > 0"></el-divider>
              <div
                v-if="device && device.videosInfo && device.videosInfo.length > 0"
                style="height: 40%; display: flex; gap: 10px; margin-bottom: 10px; align-items: center;"
              >
                <!-- 左箭头 -->
                <el-image
                  v-if="device.videosInfo.length > 2"
                  style="width: 25px; height: 25px; margin-left: 5px; flex-shrink: 0;"
                  :style="{ cursor: device.videoStartIndex <= 0 ? 'not-allowed' : 'pointer', opacity: device.videoStartIndex <= 0 ? 0.4 : 1 }"
                ...
                >
                  <div class="video-info">
                    ...
                    <div class="video-play">
                      <video
                        :ref="'video' + device.id + '-' + (device.videoStartIndex + index)"
                        width="100%"
                        height="100%"
                        controls
                        autoplay
                ...
                <el-image
                  v-if="device.videosInfo.length > 2"
                  style="width: 25px; height: 25px; margin-right: 5px; flex-shrink: 0;"
                  :style="{ cursor: device.videoStartIndex + 2 >= device.videosInfo.length ? 'not-allowed' : 'pointer', opacity: device.videoStartIndex + 2 >= device.videosInfo.length ? 0.4 : 1 }"
                  @click="device.videoStartIndex + 2 < device.videosInfo.length && scrollVideoRight(device)"
              ...
              >
                <span
                  ...
                  :style="{
                    ...
                    borderRadius: '50%',
                    margin: '0 3px',
                    backgroundColor: device.videoStartIndex === idx ? '#409EFF' : '#ccc',
                    cursor: 'pointer'
                  }"
                  @click="device.videoStartIndex = idx; handleWindowChange(device, device.videoStartIndex)"
                ></span>
              </div>
              <!-- 门 -->
              <el-divider v-if="device && device.doorsInfo && device.doorsInfo.length > 0"></el-divider>
              <div
              ...
              >
                ...
                <el-image
                  ...
                  style="width: 25px; height: 25px; margin-left: 5px; flex-shrink: 0;"
                  :style="{ cursor: device.doorStartIndex <= 0 ? 'not-allowed' : 'pointer', opacity: device.doorStartIndex <= 0 ? 0.4 : 1 }"
                ...
                <el-image
                  v-if="device.doorsInfo.length > 2"
                  style="width: 25px; height: 25px; margin-right: 5px; flex-shrink: 0;"
                  :style="{ cursor: device.doorStartIndex + 2 >= device.doorsInfo.length ? 'not-allowed' : 'pointer', opacity: device.doorStartIndex + 2 >= device.doorsInfo.length ? 0.4 : 1 }"
                  @click="device.doorStartIndex + 2 < device.doorsInfo.length && scrollDoorRight(device)"
                  :src="'/resources/rightArrow.png'"
                  fit="contain"
                />
              </div>
              ...
              >
                <span
                  ...
                  :style="{
                    ...
                    borderRadius: '50%',
                    margin: '0 3px',
                    backgroundColor: device.doorStartIndex === idx ? '#409EFF' : '#ccc',
                    cursor: 'pointer'
                  }"
                  @click="device.doorStartIndex = idx; handleWindowChange(device, device.doorStartIndex)"
                ></span>
              </div>
              <!-- 统计信息-->
                  ...
                  <el-col :span="8">
                    <div class="overview-item">
                      <div class="overview-stats">
                        <div class="stat-item">
                          <img src="/resources/people.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>人员总数：</strong></p>
                            <p class="stat-value">{{ device.userNumber }}</p>
                          </div>
                        </div>
                        <div class="stat-item">
                          <img src="/resources/people.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>已录入人脸数：</strong></p>
                            <p class="stat-value">{{ device.bindFaceUserNumber }}</p>
                          </div>
                        </div>
                        <div class="stat-item">
                          <img src="/resources/people.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>未录入人脸数：</strong></p>
                            <p class="stat-value">{{ device.userNumber - device.bindFaceUserNumber }}</p>
                          </div>
                        </div>
                      </div>
                  ...
                  <el-col :span="8">
                    <div class="overview-item">
                      <div class="overview-stats">
                        ...
                        <div class="stat-item">
                          <img src="/resources/card.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>已录入卡片数：</strong></p>
                            <p class="stat-value">{{ device.bindCardUserNumber || 0 }}</p>
                          </div>
                        </div>
                        <div class="stat-item">
                          <img src="/resources/card.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>未录入卡片数：</strong></p>
                            <p class="stat-value">{{ device.userNumber - device.bindCardUserNumber || 0 }}</p>
                          </div>
                        </div>
                      </div>
                  ...
                  <el-col :span="8">
                    <div class="overview-item">
                      <div class="overview-stats">
                        ...
                        <div class="stat-item">
                          <img src="/resources/finger.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>已录入指纹数：</strong></p>
                            <p class="stat-value">{{ device.bindFingerprintUserNumber || 0 }}</p>
                          </div>
                        </div>
                        <div class="stat-item">
                          <img src="/resources/finger.png" class="stat-icon" />
                          <div class="stat-text">
                            <p class="stat-label"><strong>未录入指纹数：</strong></p>
                            <p class="stat-value">{{ device.userNumber - device.bindFingerprintUserNumber || 0 }}</p>
                          </div>
                        </div>
                      </div>
                      <div :ref="'chart3-' + device.id" class="chart-box"></div>
                    </div>
                  </el-col>
                </el-row>
              </div>
...
export default {
  ...
  mounted() {
    ...
    window.addEventListener("peopleGate", this.handleWsEvent);
  },
  beforeDestroy() {
    window.removeEventListener("resize", this.resizeChart);
    window.removeEventListener("peopleGate", this.handleWsEvent);
  ...
  methods: {
    handleVideoClick(device, video, index) {
      ...
      if (!current) {
        // --- 当前无播放 → 开启推流 ---
        console.log("开启推流video", video);
        console.log("开启推流realIndex", realIndex);
    ...
    async startStream(deviceId, camera, realIndex) {
      ...
      try {
        ...
        if (res && res.status) {
          ...
          // 如果没上线，等待 3 秒后查询一次
          if (!data.online) {
            setTimeout(() => this.checkStreamStatus(deviceId, row.id, camera, realIndex), 3000);
          } else {
            // 已上线 → 直接播放
    ...
    async checkStreamStatus(deviceId, id, camera, realIndex, attempts = 0, config = { maxAttempts: 5, interval: 1000 }) {
      ...
      return request({
        ...
        .then((res) => {
          ...
          if (res && res.status === true) {
            ...
            // 获取设备列表后，默认选择第一个设备并获取其人数和人脸数
            if (this.devices.length > 0) {
              // 获取设备列表后，默认选择第一个设备并获取其人数和人脸数
              this.chooseDevice = String(this.devices[0].id);
              ...
              // 等待获取人数和人脸数
              Promise.all([
                this.getPeopleCount(this.devices[0].id),
                this.getFaceCount(this.devices[0].id),
              ])
                .then(([peopleCount, faceCount]) => {
                  // 给第一个设备赋值
                  device.bindCardUserNumber = peopleCount.bindCardUserNumber;
                  device.bindFaceUserNumber = peopleCount.bindFaceUserNumber;
                  device.bindFingerprintUserNumber =
                    peopleCount.bindFingerprintUserNumber;
                  device.bindPPAndPVUserNumber =
                    peopleCount.bindPPAndPVUserNumber;
                  this.$set(device, "userNumber", peopleCount.userNumber);
                  // // 人脸
                  // this.devices[0].faceCount = faceCount;
                  // 初始化第一个设备的图表
                  this.initChart(this.devices[0]);
                })
                .catch((error) => {
                  console.error("获取人数或人脸数失败：", error);
                });
              // 处理视频 // 重建数组
    ...
    handleTabClick(tab) {
      ...
      if (device) {
        ...
        // 等待获取人数和人脸数
        Promise.all([
          this.getPeopleCount(device.id),
          this.getFaceCount(device.id),
        ])
          .then(([peopleCount, faceCount]) => {
            device.bindCardUserNumber = peopleCount.bindCardUserNumber;
            device.bindFaceUserNumber = peopleCount.bindFaceUserNumber;
            device.bindFingerprintUserNumber =
              peopleCount.bindFingerprintUserNumber;
            device.bindPPAndPVUserNumber = peopleCount.bindPPAndPVUserNumber;
            this.$set(device, "userNumber", peopleCount.userNumber);
            // // 人脸
            // devicefaceCount = faceCount;
            this.initChart(device); // 初始化图表
          })
          .catch((error) => {
            console.error("获取人数或人脸数失败：", error);
          });
    ...
    // 获取人数
    async getPeopleCount(deviceId) {
      try {
        const res = await request({
          url: `/peopleInfo/count`,
          method: "POST",
          data: {
            deviceId: deviceId,
          },
        });
        console.log("获取人数结果：", res);
        // 判断请求是否成功，成功则返回人数，否则返回 0
        if (res.status) {
          return res.data; // 返回人数总数
        } else {
          this.$message.error(
            `获取人数失败 ${res.message ? "：" + res.message : ""}`,
          );
          return 0; // 请求失败时返回 0
        }
      } catch (error) {
        console.error("获取人数失败：", error);
        this.$message.error("获取人数失败，请检查网络或后端接口！");
        return 0; // 请求异常时返回 0
      }
    },
    // 获取人脸数
    async getFaceCount(deviceId) {
      try {
        const res = await request({
          url: `/faceInfo/count`,
          method: "POST",
        ...
        console.log("获取人脸数结果：", res);
        // 判断请求是否成功，成功则返回人脸数，否则返回 0
        if (res.status) {
          return res.total; // 返回人脸总数
        } else {
          this.$message.error(
            `获取人脸数失败 ${res.message ? "：" + res.message : ""}`,
          );
          return 0; // 请求失败时返回 0
        }
      } catch (error) {
        console.error("获取人脸数失败：", error);
        this.$message.error("获取人脸数失败，请检查网络或后端接口！");
        return 0; // 请求异常时返回 0
      }
    },
    initChart(device) {
      ...
      this.initUserChart(
        ...
        "指纹录入情况",
        device.bindFingerprintUserNumber,
        device.userNumber - device.bindFingerprintUserNumber,
      ); // 指纹
    },
    ...
    handleWsEvent(e) {
      const msg = e.detail;
      if (!msg || msg.type !== "peopleGate" || !msg.payload) return;
    ...
    toggleDoor(device, door, command) {
      ...
      request({
        url: "/controlPeopleDoor/controlDoorStatus",
        method: "POST",
        data: data,
      })
        .then((res) => {
